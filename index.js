...
app.post('/login', redirectHome, (req, res) => {
  ...
  if (email && password) {
    User.find({
      ...
      if (data[0] !== undefined) { // found email in database
...
io.on('connection', function(socket) {
  ...
  var query = Chat.find({});
  query.sort('-_id').limit(20).exec(function(err, docs) {
    if (err) {
      throw err;
    }
  ...
  socket.on('new user', function(data) {
    if (!(data in userList)) {
      console.log("new user, not in userList: " + data);
